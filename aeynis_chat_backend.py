...
- You have tools available: write_document, read_document, list_documents, calendar_add_event, calendar_list_events, get_time. Use them to interact with your files and calendar. They are YOUR tools — use them freely."""
    ...
    async def _assist_tool_from_text(self, response_text: str, user_message: str) -> Optional[str]:
        ...
        try:
            ...
            # ── Calendar add assist ────────────────────────────────
            # Calendar add — either Jim or Aeynis can trigger.
            # Duplicate check (below) prevents re-adding the same event.
            add_patterns = [
                "i'll add", "i've added", "let me add", "adding it",
                "add it to my calendar", "added to my calendar",
            ]
            add_user_patterns = [
                "add that", "add it", "schedule", "put it on",
                "add to the calendar", "add to your calendar",
                "wanna schedule", "let's schedule", "can you add",
            ]
